...
if(isset($_POST["do"])&&($_POST["do"]=="add_teacher_salary")){
	...
	if(mysqli_num_rows($result) > 0){
		while($row=mysqli_fetch_assoc($result)){
			...
			$student_count=$s_count+$s_count;
				
			$s_count++;
				
			$subtotal=$row['sr_fee']*$student_count;
			$subtotal1=$subtotal-($subtotal/100);
			// $total+=$subtotal1;
				
			$subtotal1 = number_format($subtotal1, 2, '.', '');
			// $total = number_format($total, 2, '.', '');
			
			$sql2="INSERT INTO teacher_salary_history(index_number,grade_id,subject_id,subject_fee,student_count,subtotal,paid,_status,month,year,date,invoice_number) 
				   VALUES ( '".$index_number."','".$grade_id."','".$subject_id."','".$subject_fee."','".$student_count."','".$subtotal1."','".$paid."','".$status."','".$month."','".$year."','".$date."','".$invoice_number."')";
...
header("Location: view/all_teacher.php?do=alert_from_salary_insert&msg=$msg&page=$page&teacher_id=$id&invoice_number=$invoice_number&desc=$status&paid=$paid");//MSK-000143-5 
}
